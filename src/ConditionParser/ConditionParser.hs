...
data HappyAbsSyn t4 t5 t6 t7 t8 t9 t10
	= HappyTerminal (ConditionParserTokens)
	| HappyErrorToken Int
	| HappyAbsSyn4 t4
	| HappyAbsSyn5 t5
	| HappyAbsSyn6 t6
...
happyExpList = Happy_Data_Array.listArray (0,48) ([0,0,0,512,0,0,0,5664,0,0,0,2576,3456,16384,40,0,41216,0,0,283,0,4096,10,322,10304,2048,49157,6,88,768,0,0,8,0,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_conditions","Grammar","Conditions","Condition","ConditionTail","Name","Comment","Formula","and","or","negation","implication","equivalence","conditionname","comment","lbrace","rbrace","literal","%eof"]
        bit_start = st * 21
        bit_end = (st + 1) * 21
...
happyReduction_7 (HappyTerminal (ConditionParser.ConditionParserTokeniser.Name happy_var_1))
	 =  HappyAbsSyn8
		 (happy_var_1
	)
...
happyReduction_8 (HappyTerminal (ConditionParser.ConditionParserTokeniser.Comment happy_var_1))
	 =  HappyAbsSyn9
		 (happy_var_1
	)
...
happyReduction_9 (HappyAbsSyn10  happy_var_3)
	...
	(HappyAbsSyn10  happy_var_1)
	 =  HappyAbsSyn10
		 ((\f x->Formula.And (happy_var_1 f x) (happy_var_3 f x))
	)
...
happyReduction_10 (HappyAbsSyn10  happy_var_3)
	...
	(HappyAbsSyn10  happy_var_1)
	 =  HappyAbsSyn10
		 ((\f x->Formula.Or (happy_var_1 f x) (happy_var_3 f x))
	)
...
happyReduction_11 (HappyAbsSyn10  happy_var_3)
	...
	(HappyAbsSyn10  happy_var_1)
	 =  HappyAbsSyn10
		 ((\f x->Formula.Or (Formula.Not (happy_var_1 f x)) (happy_var_3 f x))
	)
...
happyReduction_12 (HappyAbsSyn10  happy_var_3)
	...
	(HappyAbsSyn10  happy_var_1)
	 =  HappyAbsSyn10
		 ((\f x->Formula.Iff (happy_var_1 f x) (happy_var_3 f x))
	)
...
happyReduction_14 (HappyAbsSyn10  happy_var_2)
	_
	 =  HappyAbsSyn10
		 ((\f x->Formula.Not (happy_var_2 f x))
	)
happyReduction_14 _ _  = notHappyAtAll 

happyReduce_15 = happySpecReduce_1  10 happyReduction_15
happyReduction_15 (HappyTerminal (ConditionParser.ConditionParserTokeniser.Literal happy_var_1))
	 =  HappyAbsSyn10
		 ((\f x->Formula.Var happy_var_1 (f happy_var_1 x))
	)
...
happyNewToken action sts stk (tk:tks) =
	...
	case tk of {
	ConditionParser.ConditionParserTokeniser.And -> cont 11;
	ConditionParser.ConditionParserTokeniser.Or -> cont 12;
	ConditionParser.ConditionParserTokeniser.Not -> cont 13;
	ConditionParser.ConditionParserTokeniser.Implication -> cont 14;
	ConditionParser.ConditionParserTokeniser.Equivalence -> cont 15;
	ConditionParser.ConditionParserTokeniser.Name happy_dollar_dollar -> cont 16;
	ConditionParser.ConditionParserTokeniser.Comment happy_dollar_dollar -> cont 17;
	ConditionParser.ConditionParserTokeniser.LBrace -> cont 18;
	ConditionParser.ConditionParserTokeniser.RBrace -> cont 19;
	ConditionParser.ConditionParserTokeniser.Literal happy_dollar_dollar -> cont 20;
	_ -> happyError' ((tk:tks), [])
	}
...
happyError' :: () => ([(ConditionParserTokens)], [String]) -> HappyIdentity a
happyError' = HappyIdentity . (\(tokens, _) -> happyError tokens)
conditions tks = happyRunIdentity happySomeParser where
...
{-# LINE 8 "<command-line>" #-}
{-# LINE 1 "C:\\Users\\Z003W9PJ\\AppData\\Local\\Temp\\ghc5432_0\\ghc_2.h" #-}
